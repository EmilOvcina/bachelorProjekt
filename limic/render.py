...
COMPONENTS = (("components",), {'type': int, 'nargs': '*', 'help': "components to visualize (default: -1 for all)",
                                'metavar': 'COMPONENT'})
MARKERS = (
("--markers",), {'action': 'store_true', 'dest': 'markers', 'default': False, 'help': "include markers (HUGE FILE)"})
LINES = (("--lines",), {'action': 'store_true', 'dest': 'lines', 'default': False, 'help': "include lines (HUGE FILE)"})
HOST = (("--host",), {'type': str, 'dest': 'host', 'default': 'localhost', 'help': "hostname (default: localhost)",
                      'metavar': 'HOST'})
PORT = (
("--port",), {'type': int, 'dest': 'port', 'default': 5000, 'help': "port number (default: 5000)", 'metavar': 'PORT'})
PREFIX = (
("--prefix",), {'type': str, 'dest': 'prefix', 'default': '', 'help': "URI prefix (default: '')", 'metavar': 'PREFIX'})

CONFIG = [
    ("nx", {'help': "render HTML for NX graph", 'args': [HOST, PORT, PREFIX, LINES, MARKERS, GRAPH, HTML]}),
    ("cnx",
     {'help': "render HTML for CNX graph", 'args': [HOST, PORT, PREFIX, LINES, MARKERS, GRAPH, HTML, COMPONENTS]}),
...
def render_cnx(file_name_in, file_name_out, components=[], markers=False, lines=False, host="localhost", port=5000,
               prefix=""):
    from limic.util import start, end, load_pickled, check_overwrite
    if not check_overwrite(file_name_in, file_name_out):
        return
...
def render(g, nodes, edges, file_name_out, markers=False, lines=False, host="localhost", port=5000, prefix=""):
    ...
    if markers:
        for n in nodes:
            Marker(n[1:3], icon=BeautifyIcon(icon='none', iconStyle="opacity: 0.1;", borderColor='#7f7f00',
                                             backgroundColor='#ffff00'),
                   tooltip=("id: %d" % n[0])).add_to(m)
    if lines:
        for u, v, weight in edges:
            PolyLine([u[1:3], v[1:3]], color="#3f3f00", opacity=0.4, weight=6,
